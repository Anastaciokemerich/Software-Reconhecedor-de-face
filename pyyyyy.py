...
def getImageWithID (path ):
    ...
    for imagePaths  in imagePaths :
        faceImage = Image.open(imagePaths ).convert('L')  # Abrir imagem e converter para cinza
        ...
        ID = int(os.path .split(imagePaths )[-1].split('.')[1])    # Obter o ID da matriz
...
IDs, Facelista = getImageWithID(path )
...
for (x, y, w, h) in faces:
    ...
    for _ in range(1):#__________________________________Neurônios
        ...
        print('TREINANDO COM  ' + str(Lev) + ' NEURoooooooooooooooooooooooooooooooooooooooooooooooooooooooONIO')
